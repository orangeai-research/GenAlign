...
class GenAlignGUME(GeneralRecommender):
    ...
    def get_dynamic_agent_weights(self):
        ...
        if self.agent_warmup_epochs > 0:
            if self.current_epoch < self.agent_warmup_epochs:
                # Fusion 权重：线性增长 (0 -> target)
                ratio = self.current_epoch / self.agent_warmup_epochs
                curr_agent_weight = self.agent_weight * ratio
                # Loss 权重：直接给满，或者快速 Warm-up (这里选择直接给满，让它在幕后全力学习)
                # 如果发现初期梯度爆炸，可以改为：max(0.5, ratio) * self.agent_loss_weight
                curr_agent_loss_weight = self.agent_loss_weight 
                
        return curr_agent_weight, curr_agent_loss_weight
    ...
    def full_sort_predict(self, interaction):
        ...
        # 3. 构造 Personalized Condition (改进: 多样化 User Prior)
        # 相比单一的 Mean User Emb，我们可以生成多个典型的 User Prototypes
        # 比如 K=5 个聚类中心，或者随机采样 5 个用户
        # 这里尝试一种简单的 "Multi-Perspective" 增强：
        # 生成 Agent 时，不仅仅依赖 Mean User，而是加上一定的噪声扰动，模拟不同用户的视角
        # 然后对生成的 Agent 取平均，这样比单一 Mean 输入更鲁棒
        
        mean_user_emb = torch.mean(restore_user_e, dim=0, keepdim=True)
        
        # 采样 5 次 (Monte Carlo Integration)
        # num_samples = 5
        # accumulated_agent = 0
        # for _ in range(num_samples):
        #     # 添加高斯噪声作为 User Prior 的变化
        #     user_prior_noise = torch.randn_like(mean_user_emb) * 0.1 # std=0.1
        #     current_condition = extended_it_items + (mean_user_emb + user_prior_noise)
        #     with torch.no_grad():
        #         accumulated_agent += self.rf_agent_model.generate_agent_modality(
        #             current_condition, 
        #             t_value=self.t_agent, 
        #             steps=self.agent_step
        #         )
        # agent_modality = accumulated_agent / num_samples
        
        # 回滚到 Simple Mean (Best Performance)
    ...
    def augment_interactions(self, topk=5, conf_threshold=0.8):
        """
        利用 Agent Modality 生成虚拟交互 (Generative Data Augmentation)
        
        Process:
        1. Identify Cold Items
        2. Generate Personalized Agent Modality (using Mean User Prior)
        3. Retrieve Top-K Users close to Agent Modality
        4. Return Pseudo-Interactions
        """
        print("Start Generating virtual interactions...")
        
        # 1. 识别冷启动物品 (e.g., degree <= 5)
        # 注意：这里需要 interaction_matrix 是 scipy sparse matrix
        # self.interaction_matrix 是 coo_matrix
        # 转换为 csr 以便快速计算
        inter_csr = self.interaction_matrix.tocsr()
        item_degree = np.array(inter_csr.sum(axis=0)).flatten() # (n_items,)
        cold_items_indices = np.where(item_degree <= 5)[0] # indices
        
        if len(cold_items_indices) == 0:
            print("No cold items found (threshold<=5). Skip augmentation.")
            return []
            
        print(f"Found {len(cold_items_indices)} cold items.")
        
        cold_items_tensor = torch.LongTensor(cold_items_indices).to(self.device)
        
        # 2. 生成 Agent Modality
        # 获取多模态特征
        with torch.no_grad():
            all_embeds, _, embeds_3 = self.forward(self.norm_adj, train=True)
            explicit_image_embeds, explicit_text_embeds = embeds_3
            
            # 提取 Item 部分
            # explicit_*_embeds 是 [User, Item] 拼接的
            explicit_image_item = explicit_image_embeds[self.n_users:]
            explicit_text_item = explicit_text_embeds[self.n_users:]
            extended_it_items = (explicit_image_item + explicit_text_item) / 2
            
            # 提取 Cold Items 的多模态特征
            condition = extended_it_items[cold_items_tensor]
            
            # 加上 Mean User Prior (Personalized Trick)
            restore_user_e = all_embeds[:self.n_users]
            mean_user_emb = torch.mean(restore_user_e, dim=0, keepdim=True)
            
            # 生成 Agent
            # 注意：condition + mean_user_emb
            agent_embeds = self.rf_agent_model.generate_agent_modality(
                condition + mean_user_emb, 
                t_value=self.t_agent, 
                steps=self.agent_step
            )
            
            # 3. 检索潜在用户
            # 计算 Agent (Cold Items) 与 所有 User 的相似度
            # agent_embeds: (n_cold, dim)
            # restore_user_e: (n_users, dim)
            # 使用点积作为相似度 (与推荐打分一致)
            sim_scores = torch.matmul(agent_embeds, restore_user_e.T) # (n_cold, n_users)
            
            # 获取 Top-K 用户
            # values: (n_cold, k), indices: (n_cold, k)
            vals, indices = torch.topk(sim_scores, k=topk, dim=1)
            
            new_inters = []
            count = 0
            
            # 遍历所有 Cold Items
            for i in range(len(cold_items_indices)):
                item_id = cold_items_indices[i] # 真实 item id
                
                for k in range(topk):
                    user_id = indices[i, k].item()
                    score = vals[i, k].item()
                    
                    # 只有置信度高的才算 (这里 conf_threshold 需要根据 score 分布来定)
                    # 由于 score 是 logits，可能很大也可能很小。
                    # 简单起见，这里不设绝对阈值，或者设一个非常宽松的，主要靠 Top-K
                    # 如果需要严格，可以用 sigmoid(score) > 0.8
                    if score > conf_threshold:
                        new_inters.append((user_id, item_id))
                        count += 1
        
        print(f"Generated {count} virtual interactions.")
        return new_inters
