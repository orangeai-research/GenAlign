...
from models.modules.genalign import RectifiedFlow, ConditionalRectifiedFlow

class GenAlignSMORE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(GenAlignSMORE, self).__init__(config, dataset)
        self.sparse = True
    ...
    def calculate_loss(self, interaction):
        ...
        # 修改 forward 返回值解包，确保接收 image_embeds 和 text_embeds
        ...
        # SMORE image_embeds 是 [user, item] cat 起来的
        # 我们只需要 item 部分
        image_embeds_items = image_embeds[self.n_users:]
        text_embeds_items = text_embeds[self.n_users:]
        
        batch_img = image_embeds_items[unique_items] 
        batch_txt = text_embeds_items[unique_items] 
        ...
        # ===================================================== 
        # Target: ID Embedding (content_embeds_items)
        batch_target_id = content_embeds_items[unique_items].detach()
        
        # Condition: Multimodal Feature (side_embeds_items)
        batch_condition = side_embeds_items[unique_items].detach()
        ...
        return batch_mf_loss + batch_emb_loss + batch_reg_loss + self.cl_loss * cl_loss + loss_intra_flow + loss_agent_flow

    def full_sort_predict(self, interaction):
        user = interaction[0]

        # 1. 获取所有 Embedding (调用 forward(train=True) 以获取 side_embeds)
        # 注意：eval模式下 dropout 不会生效
        ua_embeddings, ia_embeddings, side_embeds, content_embeds, _, _ = self.forward(self.norm_adj, train=True)
        
        # 原始 SMORE 预测使用的是 ia_embeddings (all_embeds = content + side)
        # ia_embeddings = all_embeddings_items
        
        side_embeds_users, side_embeds_items = torch.split(side_embeds, [self.n_users, self.n_items], dim=0)
        
        # 2. 生成 Agent Modality
        # Condition: side_embeds_items
        condition_feat = side_embeds_items
        
        with torch.no_grad():
            agent_modality = self.rf_agent_model.generate_agent_modality(condition_feat, t_value=self.t_agent, steps=self.agent_step)

        # 3. 融合
        # 最终 Item = 原始 Item + α * Agent
        curr_agent_weight, _ = self.get_dynamic_agent_weights()
        final_item_embeddings = ia_embeddings + curr_agent_weight * agent_modality
